...
4. **Gather, per source** (use the connected Slack and Microsoft 365 tools; skip any source that isn't connected and note it):
   ...
   - **Teams** — for each **recorded** meeting the user attended in the window that has a recap/transcript, you MUST do **both** of the following. **Doing only the action-item half is the single most common failure of this skill — do not skip the meeting record.**
...
9. **Finish-check (required).** Before you end, verify and state these in your summary:
   ...
   These checks exist because skipping the meeting record, and dropping the second link on tracker tasks, are this skill's two most common failures.
